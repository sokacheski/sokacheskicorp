...
// 👈 INTERFACE CORRIGIDA: module como objeto opcional (igual ao pai)
interface Lesson {
  _id: string;
  title: string;
  order?: number;
  module?: {  // 👈 AGORA É OBJETO OPCIONAL, IGUAL AO PAI
    _id: string;
    title: string;
  };
}
...
export default function ReorderLesson({
  open,
  onClose,
  onSave,
  moduleId,
  ...
  useEffect(() => {
    ...
    } else if (open && (!lessons || lessons.length === 0)) {
      setItems([]);
    }
  }, [open, lessons]);
  ...
  const handleSave = async () => {
    if (!moduleId) {
      toast.error("ID do módulo não encontrado");
      return;
    }
            ...
            <div className="px-8 py-6 bg-[#1A212F] border-b border-gray-800 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-400 mt-1">
                  Arraste para ordenar as aulas como preferir
                </p>
              </div>
              <button
                onClick={onClose}
